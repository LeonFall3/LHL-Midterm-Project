...
def JSON_import(path):
    ...
    df=df.drop(['baths_1qtr','list_price','sub_type','price_reduced_amount','flags.is_subdivision', 'flags.is_contingent', 'flags.is_price_reduced', 'flags.is_pending', 'flags.is_foreclosure', 'flags.is_plan','listing_id','status','flags.is_new_listing','flags.is_coming_soon','location.address.coordinate','description.sold_date','list_date','location.address.state_code','last_update_date','branding', 'source','matterport', 'photos', 'virtual_tours','primary_photo.href','source.plan_id','source.agents','source.spec_id','source.type','lead_attributes.show_contact_an_agent','location.county.fips_code','products.brand_name','other_listings.rdc','primary_photo','products','other_listings', 'open_houses','community.advertisers','community.description.name','community.advertisers','location.county','flags.is_new_construction','flags.is_for_rent','name','community'], axis=1)
